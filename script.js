...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateHSCTLimitsVisibility() {
        ...
        if (isInView && !hsctLimitsVisible) {
            ...
            // After all blocks have appeared, transform into a wall after 10 seconds
            if (blocks.length > 0) {
                // Cancel any existing timeout to prevent multiple transformations
                ...
                // Store the timeout ID so we can cancel it if user scrolls away
                wallTransformationTimeout = setTimeout(() => {
                    // Only proceed with wall transformation if section is still visible
                    if (hsctLimitsVisible) {
                        // Hide the individual blocks
                        blocks.forEach(block => {
                        ...
                        // Show the wall
                        if (wallContainer) {
                            wallContainer.classList.add('visible');
                        }
                    }
                    wallTransformationTimeout = null;
                }, lastBlockVisibleTime + 10000); // 10 seconds after last block appears
            }
        } else if (!isInView && hsctLimitsVisible) {
            // Section just became invisible - reset all animations
            hsctLimitsVisible = false;
            
            // Cancel the wall transformation timeout to prevent it from firing while section is not visible
            if (wallTransformationTimeout) {
                clearTimeout(wallTransformationTimeout);
                wallTransformationTimeout = null;
            }
